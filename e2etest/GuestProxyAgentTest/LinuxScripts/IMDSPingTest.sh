...
for i in {1..10}; do
    ...
    statusCode=$(curl --ipv6 --silent --show-error --output /dev/null --write-out "%{http_code}" -H "Metadata:True" "$ipv6_dual_stack_url")
    curlExitCode=$?
    ...
    responseHeaders=$(curl --ipv6 --silent --show-error --head -H "Metadata:True" "$ipv6_dual_stack_url")
    curlExitCode=$?
    if [ $curlExitCode -ne 0 ]; then
