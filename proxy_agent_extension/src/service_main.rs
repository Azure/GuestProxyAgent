...
fn monitor_thread() {
    ...
    let mut proxy_agent_update_reported: Option<tracing::Span> = None;
    loop {
        ...
        if cache_seq_no != current_seq_no {
            ...
            if proxyagent_file_version_in_extension != proxyagent_service_file_version {
                ...
                tracing::info!(
                    "Version mismatch between file versions. ProxyAgentService File Version: {}, ProxyAgent in Extension File Version: {}", 
                ...
                proxy_agent_update_reported =
                    Some(tracing::info_span!("proxy_agent_update_reported"));
                let output = info_span!("proxy_agent_install").in_scope(move || {
                    install_command.arg("install");
                    let output = install_command.output();
                    tracing::info!("Update Proxy Agent command completed");
                    output
                });
        ...
        if status.status == *constants::SUCCESS_STATUS {
            if let Some(proxy_agent_update_reported) = proxy_agent_update_reported.as_ref() {
                let _enter = proxy_agent_update_reported.enter();
                tracing::info!("Proxy Agent Service is updated and reporting successful status");
            }
...
fn backup_proxyagent(setup_tool: &String) {
    match Command::new(setup_tool).arg("backup").output() {
        ...
        Err(e) => {
            tracing::error!("Error in running Backup Proxy Agent command: {:?}", e,);
        }
    }
}
...
fn report_proxy_agent_aggregate_status(
    ...
    match misc_helpers::json_read_from_file::<GuestProxyAgentAggregateStatus>(
        ...
        Ok(ok) => {
            tracing::info!("Successfully read proxy agent aggregate status file",);
...
fn extension_substatus(
    ...
    if proxy_agent_aggregate_status_file_version != *proxyagent_file_version_in_extension {
        ...
        tracing::info!(version_mismatch_message,);
        status.configurationAppliedTime = misc_helpers::get_date_time_string();
        status.substatus = {
            vec![
                SubStatus {
    ...
    // Success Status and report to status file for CRP to read from
            ...
            .is_empty()
        {
            ...
            tracing::info!(substatus_proxy_agent_connection_message);
        }
...
fn restore_purge_proxyagent(status: &mut StatusObj) -> bool {
    ...
    if status.status == *constants::ERROR_STATUS {
        ...
        match output {
            ...
            Err(e) => {
                tracing::error!("Error in running Restore Proxy Agent command: {}", e,);
            }
        }
        true
        ...
        match output {
            ...
            Err(e) => {
                tracing::error!("Error in running Purge Proxy Agent command: {}", e,);
            }
        }
        return true;
    } else {
...
fn report_proxy_agent_service_status(
    ...
    match output {
        Ok(output) => {
            ...
            if output.status.success() {
                tracing::info!("Update Proxy Agent command output successfully");
                status.configurationAppliedTime = misc_helpers::get_date_time_string();
        ...
        Err(e) => {
            tracing::info!("Error in running Update Proxy Agent command: {}", e,);
