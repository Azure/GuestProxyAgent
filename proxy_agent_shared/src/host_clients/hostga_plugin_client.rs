...
impl HostGAPluginClient {
            ...
            return Ok(HostGAPluginResponse {
                body: Some(
                    serde_json::from_str::<Certificates>(&certs)
                        .map_err(FormattedErrorMessage::from)?,
                ),
        ...
        for<'a> T: serde::Deserialize<'a>,
    {
        ...
        let response = if let Some(timeout_in_seconds) = self.timeout_in_seconds {
            ...
            .await
            .map_err(Into::<FormattedErrorMessage>::into)??
        } else {
        ...
        let body_string = hyper_client::read_response_body_as_string(response, "utf-8").await?;
        Err(FormattedErrorMessage {
